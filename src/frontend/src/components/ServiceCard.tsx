...
function ServiceIcon({
  ...
  if (n.includes("road") || n.includes("traffic"))
    return <Construction className={cls} />;
  if (n.includes("internet") || n.includes("network"))
    return <Wifi className={cls} />;
  if (n.includes("waste") || n.includes("garbage"))
    return <Trash2 className={cls} />;
  if (n.includes("government") || n.includes("office"))
...
export function ServiceCard({ service, index = 0 }: ServiceCardProps) {
  ...
  return (
    ...
    >
      ...
      {/* Description */}
      {service.status === "Warning" || service.status === "Interrupted" ? (
        <div
          style={{
            marginTop: "0.75rem",
            borderLeft:
