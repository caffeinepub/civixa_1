...
import { ChevronDown, Cpu, KeyRound, LogOut, Shield } from "lucide-react";
...
export function NavBar({
  ...
  const getRoleBadge = () => {
    ...
    if (session?.isModerator)
      return { label: "Moderator", color: "text-cyan-400" };
        ...
        >
          <div
            ...
            style={{
              ...
              border: "1px solid oklch(0.72 0.16 195 / 0.4)",
            }}
          >
            <Cpu
              className="w-4 h-4"
          ...
          <span
            className="civixa-title text-base sm:text-lg tracking-wider shrink-0"
          ...
          {!session && (
            ...
            >
              <Button
                variant="outline"
                size="sm"
                className="gap-2 border-white/15 hover:bg-white/5 text-sm"
              >
                <KeyRound className="w-3.5 h-3.5" />
                <span className="hidden sm:inline">Staff Login</span>
                <ChevronDown
                  className={`w-3 h-3 text-muted-foreground transition-transform duration-200 ${staffHover ? "rotate-180" : ""}`}
                />
              </Button>
              {staffHover && (
                <div className="absolute right-0 top-full pt-1 z-50">
                  <div
                    className="rounded-md border border-white/10 shadow-xl overflow-hidden"
                    style={{
                      background: "oklch(0.14 0.02 255 / 0.97)",
                      backdropFilter: "blur(16px)",
                      minWidth: "11rem",
                    }}
                  >
                    ...
                    <button
                      ...
                      className="w-full flex items-center gap-2 px-3 py-2.5 text-sm hover:bg-white/8 transition-colors text-left"
                      onClick={() => {
                        void navigate({ to: "/login/moderator" });
                        setStaffHover(false);
                      }}
                    >
          ...
          {session && (
            ...
            >
              ...
              >
                ...
                <ChevronDown
                  className={`w-3.5 h-3.5 text-muted-foreground transition-transform duration-200 ${userHover ? "rotate-180" : ""}`}
                />
              </Button>
              {userHover && (
                <div className="absolute right-0 top-full pt-1 z-50">
                  ...
                  >
                    <div className="px-3 py-2.5 border-b border-white/10">
                      ...
                      <p className={`text-xs mt-0.5 ${getRoleBadge().color}`}>
                        {getRoleBadge().label}
                      </p>
                    </div>
                    {session.isAdmin && (
