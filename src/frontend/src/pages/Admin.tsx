...
import { addAuditLog, generateId, getUsers, setUsers } from "../lib/storage";
...
export function Admin() {
  ...
  const handleStatusSelect = (
    svc: { id: string; serviceName: string; description?: string },
    newStatus: "Operational" | "Warning" | "Interrupted",
  ) => {
  ...
  const confirmStatusChange = () => {
    ...
    updateServiceStatus(
      statusChangeDialog.svcId,
      statusChangeDialog.newStatus,
      statusChangeDialog.description,
    );
    toast.success(
      `${statusChangeDialog.svcName} → ${statusChangeDialog.newStatus}`,
    );
  ...
  const [newUserRole, setNewUserRole] = useState<"admin" | "moderator">(
    "moderator",
  );
  ...
  const handleAddService = () => {
    ...
    addService(
      svcLocId,
      svcName.trim(),
      svcImpact.trim() || "General services",
    );
  ...
  const handleAddUser = () => {
    ...
    const newUser: CivixaUser = {
      ...
      ...(newUserRole === "moderator"
        ? { assignedLocationId: newUserLocId }
        : {}),
  ...
  return (
    <BackgroundLayout>
      <div className="min-h-screen overflow-x-hidden">
        ...
        <div className="max-w-7xl mx-auto px-2 sm:px-4 lg:px-6 py-8 overflow-x-hidden">
          {/* Header */}
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  System Administration — {session?.name}
                </p>
              </div>
            </div>
            <Button
                    ...
                    {locations.map((loc) => (
                      <TableRow key={loc.id} className="border-white/5">
                        ...
                        <TableCell className="text-sm">
                          {
                            services.filter((s) => s.locationId === loc.id)
                              .length
                          }
                        </TableCell>
                ...
                <Table>
                  ...
                  <TableBody>
                    {filteredServices.map((svc) => {
                      const locName =
                        locations.find((l) => l.id === svc.locationId)?.name ??
                        "—";
                      return (
                  ...
                  <TableBody>
                    {users.map((user) => {
                      const assignedLoc = user.assignedLocationId
                        ? locations.find(
                            (l) => l.id === user.assignedLocationId,
                          )
                        : null;
                      return (
                          ...
                          <TableCell>
                            {user.isAdmin ? (
                              ...
                              <Badge
                                className="text-xs"
                                style={{
                                  background: "oklch(0.72 0.16 195 / 0.15)",
                                  color: "oklch(0.72 0.16 195)",
                                  border:
                                    "1px solid oklch(0.72 0.16 195 / 0.3)",
                                }}
                              >
                                Moderator
                              </Badge>
        ...
        <Dialog
          open={!!editLocId}
          onOpenChange={(v) => !v && setEditLocId(null)}
        >
          <DialogContent className="glass-card-solid border-white/15 max-w-sm">
            <DialogHeader>
          ...
          <DialogContent className="glass-card-solid border-white/15 max-w-sm">
            ...
            <div className="space-y-3 mt-2">
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  placeholder="e.g. Affects residential zones"
                  value={svcImpact}
                  onChange={(e) => setSvcImpact(e.target.value)}
                />
              </div>
            </div>
            <DialogFooter className="mt-4">
              ...
              <Button
                ...
                disabled={!svcLocId || !svcName.trim()}
                style={{ background: "oklch(0.5 0.18 255)", color: "white" }}
              >
                Add Service
              </Button>
            ...
            <div className="space-y-3 mt-2">
              ...
              <div className="space-y-1.5">
                ...
                <Select
                  value={newUserRole}
                  onValueChange={(v) => {
                    setNewUserRole(v as "admin" | "moderator");
                    setNewUserLocId("");
                  }}
                >
                  <SelectTrigger className="h-9">
            ...
            <DialogFooter className="mt-4">
              ...
              <Button
                ...
                disabled={
                  ...
                  (newUserRole === "moderator" && !newUserLocId)
                }
                style={{ background: "oklch(0.5 0.18 255)", color: "white" }}
              >
                Add User
        ...
        <AlertDialog
          open={!!deleteLocId}
          onOpenChange={(v) => !v && setDeleteLocId(null)}
        >
          <AlertDialogContent className="glass-card-solid border-white/15">
            <AlertDialogHeader>
              <AlertDialogTitle>Delete Location?</AlertDialogTitle>
              <AlertDialogDescription>
                This will delete the location and all associated services and
        ...
        >
          <AlertDialogContent className="glass-card-solid border-white/15">
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This will permanently remove this user. They will no longer be
                able to log in.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
        ...
        <AlertDialog
          open={!!deleteSvcId}
          onOpenChange={(v) => !v && setDeleteSvcId(null)}
        >
          <AlertDialogContent className="glass-card-solid border-white/15">
            <AlertDialogHeader>
        ...
        <Dialog
          ...
          onOpenChange={(v) => !v && setStatusChangeDialog(null)}
        >
          <DialogContent className="glass-card-solid border-white/15 max-w-sm">
            <DialogHeader>
              ...
              <div className="space-y-4 mt-2">
                  ...
                  >
                    ...
                    <span
                      className={`text-xs font-semibold ${
                        statusChangeDialog.newStatus === "Operational"
                          ? "text-emerald-400"
                ...
                <div className="space-y-1.5">
                  <Label className="text-xs">
                    {statusChangeDialog.newStatus === "Warning" ||
                    statusChangeDialog.newStatus === "Interrupted" ? (
                      <>
                        Reason for closure / disruption{" "}
                  ...
                  <Textarea
                    placeholder={
                      statusChangeDialog.newStatus === "Warning" ||
                      statusChangeDialog.newStatus === "Interrupted"
                        ? "e.g. Scheduled maintenance, pipeline burst, cable fault…"
                        : "Describe the current situation, affected areas, estimated resolution…"
                    }
